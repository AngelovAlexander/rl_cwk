    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(*args, **kwargs):
            ...
            if timestep >= max_timestep * self.exploration_fraction:
                ...
                return self.epsilon_start - ((self.epsilon_start - self.epsilon_min) * timestep) / (max_timestep * self.exploration_fraction)
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if explore and np.random.rand() < self.epsilon:
            ...
            return torch.argmax(self.critics_net(torch.Tensor(obs))).item()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        max_q = torch.max(self.critics_target(batch.next_states), 1)[0].unsqueeze(1).detach()
        y_i = batch.rewards + (1 - batch.done) * self.gamma * max_q
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        ### PUT YOUR CODE HERE ###
        if explore:
            return Categorical(self.policy(torch.Tensor(obs))).sample().item()
        else:
            return torch.argmax(self.policy(torch.Tensor(obs))).item()
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        for t in reversed(range(len(actions))):
            ...
            p_loss = p_loss - g * torch.log(self.policy(torch.Tensor(observations[t]))[actions[t]])
        p_loss /= len(actions)
