        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        if random.random() < self.epsilon:
            ...
            for a in range(self.n_acts):
                q_values = np.append(q_values, self.q_table[(obs, a)])
                best_actions = [i for i in range(len(q_values)) if q_values[i] == np.max(q_values)]
...
class QLearningAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        if not done:
            for a in range(self.n_acts):
                next_q_values = np.append(next_q_values, self.q_table[(n_obs, a)])
            next_q = np.max(next_q_values)

        self.q_table[(obs, action)] = self.q_table[(obs, action)] + self.alpha * (reward + self.gamma * next_q - self.q_table[(obs, action)])
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        for t in reversed(range(len(actions))):
            ...
            if not pair in zip(obses[:t], actions[:t]):
                ...
                if pair not in self.q_table:
                    self.q_table[pair] = 0
                self.q_table[pair] = (g + self.q_table[pair] * (self.sa_counts[pair] - 1)) / self.sa_counts[pair]
