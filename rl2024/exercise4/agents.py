...
class DDPG(Agent):
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        ### PUT YOUR CODE HERE ###
        with torch.no_grad():
            #print(self.actor(torch.Tensor(obs)))
            action = self.actor(torch.Tensor(obs))#.item()
            if explore:
                action += self.noise.sample()
            action = np.clip(np.array(action, dtype = np.float32), self.lower_action_bound, self.upper_action_bound)
        #print(action)
        return action
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        y_i = batch.rewards + self.gamma * (1 - batch.done) * self.critic_target(torch.cat((batch.next_states, self.actor_target(batch.next_states).detach()), dim=1))
        q = self.critic(torch.cat((batch.states, batch.actions), dim=1))
        q_loss = torch.nn.functional.mse_loss(y_i, q)
        q_loss.backward()
        self.critic_optim.step()
        p_loss = torch.mean(-self.critic(torch.cat((batch.states, self.actor(batch.states)), dim = 1)))
        p_loss.backward()
        self.policy_optim.step()
        self.critic_target.soft_update(self.critic_target, self.tau)
        self.actor_target.soft_update(self.actor_target, self.tau)
        return {
            "q_loss": q_loss.item(),
            "p_loss": p_loss.item(),
        }
